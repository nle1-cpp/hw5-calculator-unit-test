...
void setUp(void) {
  ...
  // For example, you might reset a global value, dynamically allocate memory,
  // etc. In this example, we do not need anything here.
}
...
void test_multiply_zero(void) {
  TEST_ASSERT_EQUAL(0, multiply(0, 0)); // expect 10 * 0 = 0
  TEST_ASSERT_EQUAL(0, multiply(0, 0)); // expect 0 * 0 = 0
}

void test_multiply_overflow(void) {
  int result = multiply(INT_MAX - 5, 2); // Should wrap around or cause undefined behavior
  TEST_ASSERT_TRUE(result < 0); // This checks if overflow occurred
}

void test_multiply_underflow(void) {
  int result = multiply(INT_MIN + 5, 2);
  TEST_ASSERT_TRUE(result > 0); // This checks if underflow occurred
}
